...
class ResourceRequirementMixin(object):
    """
    Inherit from this class to add resource requirements for an object. These
    properties insure that even if the user does not pass a resource requirment, the
    default value will be available when the user's code is run in Job.run or in
    Job.Service.start. If a value is not set and the Job is not yet ready to be run, this method
    will return None.
    """
    ...
    def memory(self):
        ...
        """
        if self._memory or self._config:
    ...
    def cores(self):
        ...
        """
        if self._cores or self._config:
    ...
    def preemptable(self):
        ...
        """
        if self._preemptable or self._config:
            return self._cores if self._cores is not None else self._config.defaultPreemptable
        else:
...
class Job(ResourceRequirementMixin):
    ...
    def _createEmptyJobWrapperForJob(self, jobStore, command=None, predecessorNumber=0):
        ...
        """
        # _config must be set to determine default values for resource requirements
...
class ServiceJob(Job):
    ...
    def __init__(self, service):
        ...
        """
        # The service resource requirments are not guaranteed to be set yet so default to None
        Job.__init__(self, memory=getattr(service, 'memory', None), cores=getattr(service, 'cores', None),
                     disk=getattr(service, 'disk', None), preemptable=getattr(service, 'preemptable', None))
